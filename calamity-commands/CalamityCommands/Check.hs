...
-}
buildCheck :: (Monad m, P.Member (P.Final m) r) => S.Text -> (c -> P.Sem r (Either L.Text a)) -> P.Sem r (Check m c a)
...
buildCheckPure :: Monad m => S.Text -> (c -> Either L.Text a) -> Check m c a
buildCheckPure name cb = MkCheck name (pure . cb)

{- | Given an invokation context @c@, run a check and transform the result into an
 @'Either' 'CommandError' ()@.
-}
runCheck :: (Monad m, P.Member (P.Embed m) r) => c -> Check m c a -> P.Sem r (Either CommandError a)
runCheck ctx chk = P.embed (callback chk ctx) <&> left' (CheckError (chk ^. #name))
